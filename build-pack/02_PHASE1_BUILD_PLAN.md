...
- [ ] **1.8 Verify the timezone assumption (this decides correctness of every verdict).**
...
- [ ] **1.13 Design QA.** `playwright install chromium`; `python scripts/check.py --step 1.13` serves `site/` locally, screenshots `index.html`, one company page, one dry-day event page (or a `pending_rain_data` page if no dry-day event exists yet) and `method.html` at 1440×900 and 390×844 into `screenshots/`, and computes WCAG contrast ratios for the palette pairs in `01_SPEC.md` §7.8 from the CSS variables.
