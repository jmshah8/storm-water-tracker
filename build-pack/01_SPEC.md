...
9.3 **Workflows.**
- `poll.yml` — `schedule: cron: "*/10 * * * *"` + `workflow_dispatch`. Steps: checkout (fetch-depth 1), Python 3.12, `pip install -r requirements.txt`, then **six polls ten minutes apart in the one run** (`collect.py` + commit-if-changed each time), because GitHub starts scheduled runs far less often than every 10 minutes — it dropped all but 3 of ~45 due runs on 17 Sep 2026 (change agreed with Jaimin, 17 Sep 2026). Each poll runs commit-if-changed (`git add data && git diff --cached --quiet || git commit -m "poll: $(date -u +%FT%TZ)" && git push`). `concurrency: {group: poll, cancel-in-progress: false}` — its own lane, so holding it for ~50 minutes never blocks `rain`/`classify`; overlapping commits are handled by the `git pull --rebase` retry. Timeout 70 minutes.
