...
5.7 **Versioning.** Any change to §5.2–5.3 requires a new `rule_version` (`dry-day-v2`, …), a dated changelog entry in `NOTES_FOR_JAIMIN.md`, and full re-classification. Never re-classify old events under a new version silently. (Amended 23 Sep 2026, Jaimin's decision: the changelog was previously required on the Method page and is no longer published there. The public record of what changed remains: every event page carries the `rule_version` that produced it, and every flip between `dry day spill` and `not a dry day` is in the public `verdict_changes.csv`.)
...
9.3 **Workflows.**
- `poll.yml` — `schedule: cron: "*/10 * * * *"` + `workflow_dispatch`. Steps: checkout (fetch-depth 1), Python 3.12, `pip install -r requirements.txt`, then **one poll** (`collect.py` + commit-if-changed: `git add data && git diff --cached --quiet || git commit -m "poll: $(date -u +%FT%TZ)" && git push`, with a five-attempt `git pull --rebase` retry for overlapping commits). **The run is started by cron-job.org, which POSTs a `workflow_dispatch` every ten minutes** (20 Sep 2026, Jaimin's decision), because GitHub starts scheduled runs far less often than asked — it dropped all but 3 of ~45 due runs on 17 Sep 2026, and was still managing only about 7 starts a day on 20 Sep. The `*/10` schedule stays as a fallback for whenever GitHub does start it and for whenever the external trigger is down. Between 17 and 20 Sep 2026 one started run instead performed six polls ten minutes apart; that work-around is removed, because with a reliable trigger a 50-minute run would make every new trigger queue behind it. `concurrency: {group: poll, cancel-in-progress: false}` — its own lane. Timeout 10 minutes.
